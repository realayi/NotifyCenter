#ifndef __NOTIFYCENTERFUNCTIONAL_H__
#define __NOTIFYCENTERFUNCTIONAL_H__
...
namespace NC
{
	...
	struct arg
	{
		...
		arg(int unPos):m_unPos(unPos){}

		operator T() const
		{
			return GetCurrentArg<T>(m_unPos);
		}
	};
	...
	struct argref
	{
		...
		argref(unsigned int unPos):m_unPos(unPos){}

		operator T&() const
		{
			return GetCurrentArgRef<T>(m_unPos);
		}
	};
	...
	struct argcref
	{
		...
		argcref(unsigned int unPos):m_unPos(unPos){}

		operator const T&() const
		{
			return GetCurrentArgCRef<T>(m_unPos);
		}
	};
